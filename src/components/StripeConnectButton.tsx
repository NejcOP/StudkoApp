...
export const StripeConnectButton = ({ hasConnectAccount, isOnboardingComplete = false }: StripeConnectButtonProps) => {
  ...
  return (
    <Button
      ...
      disabled={loading}
      variant={isOnboardingComplete ? "default" : hasConnectAccount ? "outline" : "default"}
      className={`w-full ${isOnboardingComplete ? 'bg-green-600 hover:bg-green-700 text-white' : ''}`}
    >
      {loading ? (
      ...
      {isOnboardingComplete ? 'Stripe uspešno povezan ✓' : hasConnectAccount ? 'Uredi izplačila' : 'Nastavi izplačila (Stripe)'}
    </Button>
  );
};
